...
def compare_cow_transport_algorithms():
    ...
    greedyTrips = len(greedy_cow_transport(cows,limit=10))
    greedyEnd = time.time()
    greedyDuration = greedyEnd - greedyStart
    
    bruteStart = time.time()
    bruteTrips = len(brute_force_cow_transport(cows,limit=10))
    bruteEnd = time.time()
    bruteDuration = bruteEnd - bruteStart
    
    return 'Greedy trips: {}, Greedy duration: {}, Brute trips: {}, Brute duration: {}'.format(greedyTrips, greedyDuration, bruteTrips, bruteDuration)
